...
if (seeAllTicketsBtn) {
        ...
        formHTML = `
            <div class="inventory-form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="form-name" name="name" value="${safeValue(existingData.name)}" required>
                </div>

                <div class="form-group">
                    <label for="form-email">Correo electrónico</label>
                    <input type="email" id="form-email" name="email" value="${safeValue(existingData.email)}">
                </div>

                <div class="form-group">
                    <label for="form-area">Rol / Área</label>
                    <input type="text" id="form-area" name="area" value="${safeValue(existingData.area || existingData.role)}" placeholder="Ej: Administrativo, Ventas, Tecnología">
                </div>

                <div class="form-group">
                    <label for="form-locationId">Ubicación principal</label>
                    <select id="form-locationId" name="locationId">
                        ${locationOptions}
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="form-status" name="status">
                        <option value="Activo" ${(existingData.status || 'Activo') === 'Activo' ? 'selected' : ''}>Activo</option>
                        <option value="Inactivo" ${existingData.status === 'Inactivo' ? 'selected' : ''}>Inactivo</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="form-registerDate">Fecha de registro</label>
                    <input type="date" id="form-registerDate" name="registerDate" value="${safeValue(existingData.registerDate)}">
                </div>
            </div>
        `;
    }
    ...
    if (isLocation) {
        formHTML = `
            <div class="inventory-form-grid">
                <div class="form-group">
                    <label for="form-name">Nombre de la ubicación</label>
                    <input type="text" id="form-name" name="name" value="${safeValue(existingData.name)}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="form-type" name="type">
                        <option value="Administrativo" ${(existingData.type || '') === 'Administrativo' ? 'selected' : ''}>Administrativo</option>
                        <option value="Bodega" ${(existingData.type || '') === 'Bodega' ? 'selected' : ''}>Bodega</option>
                        <option value="Punto de venta" ${(existingData.type || '') === 'Punto de venta' ? 'selected' : ''}>Punto de venta</option>
                        <option value="Servicio técnico" ${(existingData.type || '') === 'Servicio técnico' ? 'selected' : ''}>Servicio técnico</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="form-city">Ciudad</label>
                    <input type="text" id="form-city" name="city" value="${safeValue(existingData.city || 'Cali')}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="form-address" name="address" value="${safeValue(existingData.address)}">
                </div>
                ...
                .then(snapshot => {
                    ...
                    ticketsData.forEach(ticket => {
                        const ticketDate = ticket.createdAt
                            ? ticket.createdAt.toDate().toLocaleDateString('es-ES')
                            : 'Fecha N/A';
                            ...
                            <li style="display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid #eee;">
                                <div style="display:flex; flex-direction:column; gap:4px; max-width:75%;">
                                    <div style="font-weight:600; color:#1e293b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                        <a href="#" class="view-ticket-btn" data-id="${ticket.id}" style="color:#2563eb !important; background:transparent !important; padding:0 !important; border:none !important; box-shadow:none !important; text-decoration:underline !important; font-size:14px;">
                                            #${ticket.id}
                                        </a>
                                        <span style="color:#475569; font-weight:normal; font-size:14px;">
...
modalForm.addEventListener('submit', async (e) => {
    ...
    if (submitBtn) {
        ...
        submitBtn.textContent = isEditing ? 'Guardando cambios...' : 'Guardando...';
    }
    ...
    formData.forEach((value, key) => {
        const fieldConfig = config && config.fields ? config.fields[key] : null;

        if (fieldConfig && fieldConfig.type === 'text' && fieldConfig.optionsSource === 'computers-inventory') {
            data[key] = value && value.includes(':') ? value.split(':')[0].trim() : value;
        } else {
            data[key] = value;
        }
    });
    ...
    try {
        if (isEditing) {
            if (type === 'credentials' && category === 'software') {
                ...
                if (newComputerId !== oldComputerId) {
                    await db.runTransaction(async (transaction) => {
                        ...
                        if (oldComputerId) {
                            transaction.update(db.collection('inventory').doc(oldComputerId), { os: null });
                        }

                        if (newComputerId) {
                            transaction.update(db.collection('inventory').doc(newComputerId), { os: docId });
                        }
                    });
                } else {
                ...
                if (newLicenseId !== oldLicenseId) {
                    await db.runTransaction(async (transaction) => {
                        ...
                        if (oldLicenseId) {
                            transaction.update(db.collection('credentials').doc(oldLicenseId), { assignedTo: null });
                        }

                        if (newLicenseId) {
                            transaction.update(db.collection('credentials').doc(newLicenseId), { assignedTo: docId });
                        }
                    });
                } else {
                    await db.collection(collectionName).doc(docId).update(data);
            ...
            if (type === 'inventory' || type === 'credentials' || type === 'services') {
                ...
                if (!prefix || !counter) {
                    throw new Error('Falta prefix o counter en la configuración.');
                }
